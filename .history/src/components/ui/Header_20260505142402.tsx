...
const Header = ({ onMenuClick }: { onMenuClick?: () => void }) => {
  ...
  return (
    <header
      className={`sticky top-0 z-40 backdrop-blur  text-card-foreground border-border duration-300 transition-all ${
        isScrolled
          ? "bg-card/80 backdrop-blur-md shadow-md"
          : "bg-card/70 backdrop-blur"
      }`}
    >
      <div className="flex items-center justify-between px-6 py-3 px-4 md:px-6 lg:px-8 py-3">
        {/* LEFT */}

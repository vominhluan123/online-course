...
import { CourseClient, CourseStatus, statusLabel } from "@/types/course";
...
const CourseUpdate = ({ course }: { course: CourseClient }) => {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      slug: course.slug ?? "",
      image:
        course.image && course.image.trim() !== "" ? course.image : undefined,
    ...
    <form id="form-rhf-input" onSubmit={form.handleSubmit(onSubmit)}>
      <FieldGroup>
        <div className="grid lg:grid-cols-2 grid-cols-1 gap-8 mt-8">
          ...
          {/* ==================== PHẦN ẢNH KHÓA HỌC ==================== */}
          <Field>
            <FieldLabel>Ảnh khóa học</FieldLabel>
              ...
              {!imgWatch ? (
                <div className="space-y-3">
                  <UploadButton
                    ...
                    appearance={{
                      button:
                        "ut-ready:bg-primary outline-none ut-uploading:cursor-not-allowed ut-uploading:opacity-50",
                    }}
                  ...
                  {isUploading && (
                    <div className="relative aspect-video overflow-hidden rounded-xl border bg-skeleton bg-black/10  dark:bg-white/10">
          ...
          <Field>
            ...
            <div className="space-y-3">
              {requirementFields.map((item, index: number) => (
                <div key={item.id} className="flex gap-2">
                  ...
                  <Button
                    ...
                    variant="destructive"
                    onClick={() => {
                      const removedItem = requirementFields[index];
                      removeRequirement(index);
                      toast("Đã xoá", {
                        action: {
                          label: "Hoàn tác",
                          onClick: () => appendRequirement(removedItem),
                        },
                      });
                    }}
                  >
              ...
              <Button
                ...
                variant="outline"
                onClick={() =>
                  appendRequirement({ value: "" }, { shouldFocus: true })
                }
              >
                + Thêm yêu cầu
              </Button>
          ...
          <Field>
            ...
            <div className="space-y-3">
              {benefitFields.map((item, index: number) => (
                <div key={item.id} className="flex gap-2">
                  <Input
                  ...
                  <Button
                    ...
                    variant="destructive"
                    onClick={() => {
                      const removedItem = benefitFields[index];
                      removeBenefit(index);
                      toast("Đã xoá", {
                        action: {
                          label: "Hoàn tác",
                          onClick: () => appendBenefit(removedItem),
                        },
                      });
                    }}
                  >

...
const CourseUpdate = ({ course }: { course: CourseClient }) => {
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      info: {
        requirements: (course.info?.requirements || []).map((v) => ({
          value: v,
        })),
        benefits: (course.info?.benefits || []).map((v) => ({ value: v })),
      },
    },
    mode: "onBlur",
  });
            ...
            <div className="space-y-3">
              {requirementFields.map((item, index) => (
                <div key={item.id} className="flex gap-2">
                  <Input
                    {...form.register(`info.requirements.${index}`.va)}
            ...
            render={({ field, fieldState }) => (
              <Field data-invalid={fieldState.invalid}>
                ...
                <Textarea
                  ...
                  placeholder="Có thêm kiến thức về..."
                />
                {fieldState.invalid && (
                  <FieldError errors={[fieldState.error]} />
                )}

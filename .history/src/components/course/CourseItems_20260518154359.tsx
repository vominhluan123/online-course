...
const CourseItems = ({ course }: { course: CourseSchemaType }) => {
  ...
  return (
    <article className="bg-card border text-card-foreground p-3 md:p-4 rounded-xl border-border flex flex-col h-full">
      <Link
      ...
      <div className="pt-4 flex flex-col flex-1 min-w-0">
        ...
        <div className="flex items-center justify-between mt-auto gap-10">
          ...
          <div className="flex flex-col items-end sm:flex-row sm:items-center gap-1 sm:gap-2">
            <span className="text-primary font-bold text-base md:text-lg font-heading">
        ...
        <Link
          href={`/course/${course.slug}`}
          className="flex items-center w-full justify-center mt-10 rounded-lg font-semibold font-heading bg-primary text-primary-foreground h-10 md:h-12"
        >
          Xem chi tiết
        </Link>
      </div>

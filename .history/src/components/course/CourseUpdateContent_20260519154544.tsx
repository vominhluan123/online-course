...
import { BookOpen, CirclePlay, Clock3, FileText, Lock, Pencil, Trash2 } from "lucide-react";
    ...
    >
      {chapters.map((chapter, chapterIndex) => (
        ...
        >
          ...
          <AccordionContent>
            <div className="border-t bg-muted/20 border-border">
              {chapter.lessons.map((lesson, lessonIndex) => (
                <div
                  key={lesson.title}
                  className="flex items-center justify-between gap-4 border-b px-5 py-4 last:border-b-0 hover:bg-muted/40 border-border"
                >
                  <div className="flex items-center gap-4">
                    <div className="flex size-8 items-center justify-center border-border rounded-full border bg-card text-sm font-medium">
                      {lessonIndex + 1}
                    </div>

    ...
    <div className="bg-card border text-card-foreground p-4 rounded-2xl border-border flex flex-col">
      ...
      <div className="pt-4 text-muted-foreground  flex-1 flex flex-col">
        <div className="flex items-center justify-between">
          <p className="font-bold text-lg mb-5 text-primary font-heading">
            {course.title}
          </p>
          {CourseInfo.map((item, index) => (
            <div
              className="py-1 gap-1 justify-center px-3 flex items-center justify-center bg-accent text-accent-foreground rounded-full"
              key={index}
            >
              {item.icon("size-4")}

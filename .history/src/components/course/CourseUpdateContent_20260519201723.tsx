...
import { CourseTypeModel, LectureTypeModel } from "@/lib/db";
...
const CourseUpdateContent = ({ course }: { course: CourseTypeModel }) => {
  ...
  return (
    <>
      {letures.map((leture: LectureTypeModel) => (
        <Accordion type="single" collapsible className="space-y-4">
          {letures.map((lecture: LectureTypeModel, lectureIndex: number) => (
            <AccordionItem
              key={lecture._id}
            ...
            >
              ...
              <AccordionContent>
                <div className="border-t border-border bg-muted/20">
                  {/* lessons */}
                  {lecture.lessons?.map((lesson: any, lessonIndex: number) => (
                    <div
                      key={lesson._id}

...
const page = async ({ params }: { params: Promise<{ slug: string }> }) => {
  ...
  const hasDiscount =
    price !== undefined && salePrice !== undefined && salePrice < price;
  return (
        ...
        <div className="bg-card text-card-foreground rounded-lg p-5 mt-5">
          <div className="flex flex-col">
            <div className="flex flex-wrap gap-2 mb-5 items-center">
              {hasDiscount ? (
                <>
                 <span className="text-muted-foreground font-semibold text-xl">{salePrice} VNĐ</span>
                  <span className="text-muted-foreground line-through text-sm">{price} VNĐ</span>
                  <strong>
                    {Math.floor(((price - salePrice) / price) * 100)}%
                  </strong>
                </>
              ) : (
                <span>{price ?? "Liên hệ"} VNĐ</span>
              )}
              ...
              {course.price && course.sale_price && (
                <strong>
                  {Math.floor(
                    ((course.price - course.sale_price) / course.price) * 100,
                  )}
                  %
                </strong>
              )}

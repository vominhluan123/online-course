    ...
    <section className="py-8">
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3">
        {courses.map((studyCourse) => (
          <Card
            key={studyCourse._id.toString()}
            className="overflow-hidden transition-all duration-300 hover:-translate-y-1 hover:shadow-lg ring-0"
          >
            <Image
              src={studyCourse.course.image || "/no-image.png"}
              alt={studyCourse.course.title}
              width={800}
              height={500}
            ...
            <CardContent className="space-y-4 p-5">
              <div className="flex items-start justify-between gap-3">
                <h2 className="line-clamp-2 font-semibold">
                  {studyCourse.course.title}
                </h2>

                {studyCourse.completed ? (
                  <Badge className="rounded-lg bg-green-50 text-green-700 dark:bg-green-950 dark:text-green-300">
                    Đã hoàn thành
                  </Badge>
                ) : studyCourse.percent === 0 ? (
                  <Badge className="rounded-lg bg-gray-50 text-gray-700 dark:bg-gray-900 dark:text-gray-300">
                    Chưa bắt đầu
                  </Badge>
                ) : (
              ...
              <div>
                <div className="mb-2 flex items-center justify-between text-sm">
                  ...
                  <span className="font-medium">{studyCourse.percent}%</span>
                </div>

                <Progress value={studyCourse.percent} className="h-2" />
              </div>
              ...
              >
                <Link
                  target="_blank"
                  href={`/course/${studyCourse.course.slug}/learn/${
                    studyCourse.currentLesson ?? ""
                  }`}
                >
                  {studyCourse.completed ? "Hoàn thành" : "Tiếp tục học"}
                </Link>
              </Button>
            </CardContent>
          </Card>
